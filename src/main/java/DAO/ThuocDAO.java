...
public class ThuocDAO implements ThuocService{
	...
	@Override
	public LoThuoc getLo(String ma) {
		Session session = sessionFactory.getCurrentSession();
		Transaction transaction = session.getTransaction();
		//String sql ="delete from LoThuoc where LoThuoc.maLoThuoc ='"+maLoThuoc+"'";
		try {
			transaction.begin();
			LoThuoc lo = session.get(LoThuoc.class, ma);
			transaction.commit();
			return lo;
		} catch (Exception e) {
			// TODO: handle exception
